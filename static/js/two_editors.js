var postAceInit = function(hook, context){
  ...
  if(two_editors){
    if(two_editors.onByDefault){ // Setup testing else poop out
      if(two_editors.onByDefault === 'true'){
        enabletwo_editors();
        showtwo_editors();
      }
    }
...
function enabletwo_editors(){
  ...
  if($("#two_editors").length === 0){ // If it's not available already then two_editors it
    $("#editorcontainer").prepend("<div id=two_editors><iframe id='two_editorsEmbed' src='//"+two_editors_host+"/d/"+padID+"?authorName="+authorName+"&authorColor="+authorColor+"' width='100%' height='100%' style='border:none' frameborder='0' scrolling='no'></iframe></div>");
  }
...
function showtwo_editors(){
  $("#two_editors").css({"z-index":"999999", "position":"absolute", "top":"0px", "right":"0px", "height":"200px", "width":"200px", "border":"1px solid #ccc"}).show();
